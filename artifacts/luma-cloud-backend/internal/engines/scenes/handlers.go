...
func (h *Handler) Get(c *gin.Context) {
	...
	if err != nil {
		httputil.Fail(c, http.StatusBadRequest, httputil.ErrValidation, "invalid scene id", nil)
		return
	}

	dto, err := h.svc.Get(c.Request.Context(), id, userID)
	if err != nil {
		httputil.Fail(c, http.StatusNotFound, httputil.ErrNotFound, "scene not found or unauthorized", nil)
		return
	}
